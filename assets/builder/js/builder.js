( function($) {
	...
	var Builder = {
							...
							success: function(response) {
								...
								try {
										if (response.status === 'success') {
											...
											// success response
											Builder.displayToast('success', response.toast_header_title, response.toast_body_title, $('#joinotify-automations-builder'));
											...
											// error response
											Builder.displayToast('error', response.toast_header_title, response.toast_body_title, $('#joinotify-automations-builder'));
										}
								} catch (error) {
										console.log(error);
								}
		 ...
		 * @package MeuMouse.com
		 */
		displayToast: function(type, header_title, body_title, container) {
			var toast_class = '';
			var header_class = '';
			var icon = '';
		 ...
		 * @package MeuMouse.com
		 */
		startWorkflowsTemplates: function() {
		...
		/**
			...
			* @since 1.0.0
			*/
		adjustConditionContainerHeight: function() {
			...
			$(document).on('click', '.exclude-action', function(e) {
				...
				$.ajax({
					...
					complete: function() {
						...
						Builder.adjustConditionContainerHeight();
					},
				});
			});
		},
		 ...
		 * @since 1.0.0
		 */
		includeBootstrapDatePicker: function() {
			/**
		 ...
		 * @since 1.0.0
		 */
		activateWorkflowSteps: function() {
			// return to start
	...
	$(document).ready( function() {
		Builder.init();
		Builder.uploadTemplates();
		Builder.initializeBootstrapComponents();
		Builder.preventReloadPage();
		Builder.startWorkflowsTemplates();
		Builder.addTriggerAction();
		Builder.adjustConditionContainerHeight();
		Builder.removeTriggerAction();
		Builder.removeAction();
		Builder.changeWorkflowStatus();
		Builder.loadWorkflowData();
		Builder.updateWorkflowTitle();
		Builder.changeVisibilityForElements();
		Builder.includeBootstrapDatePicker();
		Builder.whatsappMessageInput();
		Builder.exportWorkflowFile();
		Builder.activateWorkflowSteps();
		Builder.openMediaLibrary();
		Builder.sendMessageTest();
		Builder.dismissPlaceholdersTip();
		Builder.fetchAllGroups();
		Builder.saveActionEdition();
		Builder.snippetsPHPAction();
