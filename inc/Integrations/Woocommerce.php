...
class Woocommerce extends Integrations_Base {
    ...
    public function __construct() {
        ...
        if ( class_exists('WooCommerce') ) {
            add_action( 'Joinotify/Builder/Triggers', array( $this, 'add_woocommerce_triggers' ), 40 );
            add_action( 'Joinotify/Builder/Triggers_Content', array( $this, 'add_woocommerce_tab_content' ) );
            add_filter( 'Joinotify/Builder/Placeholders_List', array( $this, 'woocommerce_placeholders' ), 10, 1 );
        }
    }
     ...
     * @return void
     */
    public function add_woocommerce_tab_content() {
        if ( Admin::get_setting('enable_woocommerce_integration') === 'yes' ) : ?>
            <div id="woocommerce" class="nav-content triggers-group">
                <?php foreach ( Core::get_triggers_by_context('woocommerce') as $trigger ) : ?>
                    <div class="trigger-item <?php echo ( isset( $trigger['class'] ) ? $trigger['class'] : '' ) ?>" data-context="woocommerce" data-trigger="<?php echo esc_attr( $trigger['data_trigger'] ); ?>">
     ...
     * @return array
     */
    public function woocommerce_placeholders( $placeholders ) {
        $placeholders['{{ wc_order_number }}'] = array(
            'description' => esc_html__( 'Para recuperar o número do pedido do WooCommerce', 'joinotify' ),
        );
    
        $placeholders['{{ wc_order_status }}'] = array(
            'description' => esc_html__( 'Para recuperar o status do pedido do WooCommerce', 'joinotify' ),
        );
    
        $placeholders['{{ wc_order_date }}'] = array(
            'description' => esc_html__( 'Para recuperar a data do pedido do WooCommerce', 'joinotify' ),
        );
    
        $placeholders['{{ wc_purchased_items }}'] = array(
            'description' => esc_html__( 'Para recuperar cada produto e quantidade adquiridos no pedido, separados por linha', 'joinotify' ),
        );  
    
        $placeholders['{{ wc_payment_url }}'] = array(
            'description' => esc_html__( 'Para recuperar a URL de pagamento do pedido', 'joinotify' ),
        );
    
        $placeholders['{{ wc_full_address }}'] = array(
            'description' => esc_html__( 'Para recuperar o endereço completo do usuário', 'joinotify' ),
        );

        $placeholders['{{ wc_currency_symbol }}'] = array(
            'description' => esc_html__( 'Para recuperar o símbolo de moeda', 'joinotify' ),
        );
     ...
     */
    public static function get_full_address( $order ) {
        $address = array(
