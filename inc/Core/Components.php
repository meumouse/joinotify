...
/**
 * Frontend visual components class
 * 
 * @since 1.0.0
...
class Components {
    ...
    public static function get_action_html( $post_id, $action_name, $description = '', $action_id = '' ) {
        foreach ( Core::get_all_actions() as $action => $value ) {
                    ...
                    $html .= '<div class="modal fade" id="edit_condition_'. esc_attr( $action_id ) .'" tabindex="-1" aria-labelledby="edit_condition_'. esc_attr( $action_id ) .'_label" aria-hidden="true">';
                        $html .= '<div class="modal-dialog modal-dialog-centered">';
                            $html .= '<div class="modal-content">';
                                ...
                                $html .= '<div class="modal-body py-3 my-3">';
                                    $get_condition = Core::get_condition_item( $post_id, $action_id );
                                    $html .= self::render_condition_settings( $get_condition );
                                $html .= '</div>';

                                $html .= '<div class="modal-footer">';
                                    $html .= '<button type="button" class="btn btn-outline-secondary me-3" data-bs-dismiss="modal">'. esc_html__( 'Cancelar', 'joinotify' ) .'</button>';
                                    $html .= '<button type="button" class="btn btn-primary save-condition" data-condition-id="'. esc_attr( $action_id ) .'">'. esc_html__( 'Salvar alterações', 'joinotify' ) .'</button>';
                                $html .= '</div>';
                            $html .= '</div>';
                        $html .= '</div>';
                    $html .= '</div>';
                    ...
                    $html .= '<div class="modal fade" id="edit_action_'. esc_attr( $action_id ) .'" tabindex="-1" aria-labelledby="edit_action_'. esc_attr( $action_id ) .'_label" aria-hidden="true">';
                        $html .= '<div class="modal-dialog modal-dialog-centered">';
                            $html .= '<div class="modal-content">';
                                ...
                                $html .= '<div class="modal-body py-3 my-3">';
                                    $html .= self::get_action_settings( $post_id, $value['action'] );
                                $html .= '</div>';
    ...
    public static function get_action_condition_html( $post_id, $condition_data, $title = '', $description = '' ) {
        ...
        foreach ( $condition_data as $action ) {
            foreach ( Core::get_all_actions() as $action_index => $action_value ) {
                        ...
                        $html .= '<div class="modal fade" id="edit_action_'. esc_attr( $condition_data['data']['action'] ) .'" tabindex="-1" aria-labelledby="edit_action_'. esc_attr( $condition_data['data']['action'] ) .'_label" aria-hidden="true">';
                            $html .= '<div class="modal-dialog modal-dialog-centered">';
                                $html .= '<div class="modal-content">';
                                    ...
                                    $html .= '<div class="modal-footer">';
                                        $html .= '<button type="button" class="btn btn-outline-secondary me-3" data-bs-dismiss="modal">'. esc_html__( 'Cancelar', 'joinotify' ) .'</button>';
                                        $html .= '<button type="button" class="btn btn-primary save-condition" data-condition-id="'. esc_attr( $condition_data['data']['action'] ) .'">'. esc_html__( 'Salvar alterações', 'joinotify' ) .'</button>';
                                    $html .= '</div>';
                                $html .= '</div>';
                            $html .= '</div>';
                        $html .= '</div>';
     ...
     * @return string
     */
    public static function get_action_settings( $post_id, $action ) {
        $html = '';
        ...
        switch ( $action ) {
            case 'time_delay':
                $html .= '<div class="mb-4">';
                    $html .= '<span class="fs-md text-muted mb-2 ms-2 d-block">'. esc_html__( 'Selecione o tipo de atraso da próxima ação', 'joinotify' ) .'</span>';
                        $html .= '<select class="form-select set-time-delay-type">';
                            $html .= '<option value="period">'. esc_html__( 'Esperar tempo', 'joinotify' ) .'</option>';
                            $html .= '<option value="date">'. esc_html__( 'Esperar até uma data', 'joinotify' ) .'</option>';
                    $html .= '</select>';
                $html .= '</div>';
                ...
                $html .= '<div class="wait-time-period-container">';
                    ...
                    $html .= '<div class="input-group">';
                        $html .= '<input type="number" class="form-control get-wait-value"/>'; 
                        $html .= '<select class="form-select get-wait-period">';
                            $html .= '<option value="seconds">'. esc_html__( 'Segundo (s)', 'joinotify' ) .'</option>';
                            $html .= '<option value="minute">'. esc_html__( 'Minuto (s)', 'joinotify' ) .'</option>';
                            $html .= '<option value="hours">'. esc_html__( 'Hora (s)', 'joinotify' ) .'</option>';
                            $html .= '<option value="day">'. esc_html__( 'Dia (s)', 'joinotify' ) .'</option>';
                            $html .= '<option value="week">'. esc_html__( 'Semana (s)', 'joinotify' ) .'</option>';
                            $html .= '<option value="month">'. esc_html__( 'Mês (es)', 'joinotify' ) .'</option>';
                            $html .= '<option value="year">'. esc_html__( 'Ano (s)', 'joinotify' ) .'</option>';
                        $html .= '</select>';
                    $html .= '</div>';
                $html .= '</div>';
            ...
            case 'send_whatsapp_message_text':
                $html .= '<div class="preview-whatsapp-message-sender"></div>';
                ...
                $html .= '<div class="input-group mb-3">';
                    ...
                    $html .= '<select id="joinotify_get_whatsapp_phone_sender_edit" class="form-select">';
                        foreach ( get_option('joinotify_get_phones_senders') as $phone ) {
                            $html .= '<option value="'. esc_attr( $phone ) .'" class="get-sender-number">'. esc_html( Helpers::format_phone_number( $phone ) ) .'</option>';
                        }
                    $html .= '</select>';
                $html .= '</div>';
                ...
                $html .= '<div class="input-group mb-3">';
                    ...
                    $html .= '<input id="joinotify_get_whatsapp_number_msg_text_edit" type="text" class="form-control" value="{{ phone }}" placeholder="'. esc_attr__( '+5541987111527', 'joinotify' ) .'"/>';
                $html .= '</div>';
                ...
                $html .= '<div class="input-group mb-3">';
                    ...
                    $html .= '<input type="text" id="joinotify_get_whatsapp_message_text_edit" class="form-control set-whatsapp-message set-whatsapp-message-text" placeholder="'. esc_attr__( 'Mensagem', 'joinotify' ) .'"/>';
                $html .= '</div>';
                        ...
                        $html .= '<div id="whatsapp_msg_text_placeholder_collapse_edit" class="accordion-collapse collapse" data-bs-parent="#whatsapp_msg_text_placeholder_accordion_edit">';
                            $html .= '<div class="accordion-body">';
                                $html .= '<div class="mt-3 placeholders-list">';
                                    $trigger = Core::get_trigger_from_post( $post_id );
    ...
    public static function get_trigger_html( $context, $data_trigger, $trigger_id = '' ) {
        $trigger = Core::get_trigger( $context, $data_trigger );
    ...
    public static function build_condition_connector( $post_id, $action_id ) {
        $condition_data = Core::find_condition_by_id( $post_id, $action_id );
