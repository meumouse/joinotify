...
class License {
    ...
    private function decrypt( $encrypted, $password = '' ) {
        ...
        $plugin_log_file = 'joinotify-log';
        $logger->info('(Joinotify) Response encrypted: ' . print_r( $encrypted, true ), array('source' => $plugin_log_file));
        ...
        if ( is_string( $encrypted ) ) {
            ...
            if ( $plaintext === false ) {
                $logger->info('(Joinotify) Falha na descriptografia. Input: $encrypted: ' . print_r( $plaintext, true ), array('source' => $plugin_log_file));
            ...
            $logger->info('(Joinotify) A entrada para decrypt não é uma string. Tipo: ' . gettype( $encrypted ), array('source' => $plugin_log_file));
    ...
    private function process_response( $response ) {
        ...
        if ( ! empty( $response ) ) {
            ...
            $logger->info('(Joinotify) Response: ' . print_r( $response, true ), array('source' => $plugin_log_file));
            ...
            if ( ! empty( $this->product_key ) ) {
                ...
                $logger->info('(Joinotify) Decrypted response: ' . print_r( $decrypted_response, true ), array('source' => $plugin_log_file));
            ...
            $logger->info('(Joinotify) Response decoded: ' . print_r( $decoded_response, true ), array('source' => $plugin_log_file));
    ...
    private function _request( $relative_url, $data, &$error = '' ) {
        ...
        if ( false === $cached_response ) {
            ...
            if ( function_exists('wp_remote_post') ) {
                ...
                $logger->info('(Joinotify) Request response: ' . print_r( $server_response, true ), array('source' => $plugin_log_file));
    ...
    final function deactive_license_process( &$message = '' ) {
        ...
        if ( ! empty( $old_response->is_valid ) ) {
            if ( ! empty( $old_response->license_key ) ) {
                ...
                $logger = wc_get_logger();
                $plugin_log_file = 'joinotify-log';
                $logger->info('(Joinotify) Deactive response object: ' . print_r( $response, true ), array('source' => $plugin_log_file));
