<?php

namespace MeuMouse\Joinotify\Integrations;

use MeuMouse\Joinotify\Admin\Admin;
use MeuMouse\Joinotify\Core\Helpers;

// Exit if accessed directly.
defined('ABSPATH') || exit;

/**
 * Add integration with WhatsApp
 * 
 * @since 1.1.0
 * @version 1.4.7
 * @package MeuMouse\Joinotify\Integrations
 * @author MeuMouse.com
 */
class Whatsapp extends Integrations_Base {

    /**
     * Construct function
     *
     * @since 1.1.0
     * @version 1.4.7
     * @return void
     */
    public function __construct() {
        // add integration on settings
        add_filter( 'Joinotify/Settings/Tabs/Integrations', array( $this, 'add_integration_item' ), 10, 1 );

        // add WhatsApp message actions
        if ( Admin::get_setting('enable_whatsapp_integration') === 'yes' ) {
            add_filter( 'Joinotify/Builder/Actions', array( $this, 'add_whatsapp_messages' ), 10, 1 );
            add_filter( 'Joinotify/Builder/Action_Categories', array( $this, 'add_whatsapp_category' ), 10, 1 );
        }
    }


    /**
     * Register the "WhatsApp" category on the builder actions library
     *
     * @since 1.4.7
     * @param array $categories | Current categories
     * @return array
     */
    public function add_whatsapp_category( $categories ) {
        $categories[] = array(
            'id' => 'messages',
            'label' => __( 'Messages', 'joinotify' ),
            'icon' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M20 2H4c-1.103 0-2 .897-2 2v18l5.333-4H20c1.103 0 2-.897 2-2V4c0-1.103-.897-2-2-2zm0 14H6.667L4 18V4h16v12z"></path><path d="M7 7h10v2H7zm0 4h7v2H7z"></path></svg>',
            'priority' => 20,
        );

        return $categories;
    }


    /**
     * Add integration item on settings
     * 
     * @since 1.3.0
     * @param array $integrations | Current integrations
     * @return array
     */
    public function add_integration_item( $integrations ) {
        $integrations['whatsapp'] = array(
            'title' => __('WhatsApp', 'joinotify'),
            'description' => __( "Use the world's largest messaging service to notify your customers about events on your website.", 'joinotify'),
            'category' => 'channels',
            'settings' => self::get_integration_settings(),
            'modal' => array(
                'title' => __( 'WhatsApp settings', 'joinotify' ),
                'description' => __( 'Connect this site to your Joinotify account to deliver messages through the official WhatsApp Cloud API.', 'joinotify' ),
                'button_label' => __( 'Configure', 'joinotify' ),
                'size' => 'medium',
            ),
            'icon' => '<svg viewBox="-2.73 0 1225.016 1225.016" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"><g stroke-width="0"></g><g stroke-linecap="round" stroke-linejoin="round"></g><g><path fill="#E0E0E0" d="M1041.858 178.02C927.206 63.289 774.753.07 612.325 0 277.617 0 5.232 272.298 5.098 606.991c-.039 106.986 27.915 211.42 81.048 303.476L0 1225.016l321.898-84.406c88.689 48.368 188.547 73.855 290.166 73.896h.258.003c334.654 0 607.08-272.346 607.222-607.023.056-162.208-63.052-314.724-177.689-429.463zm-429.533 933.963h-.197c-90.578-.048-179.402-24.366-256.878-70.339l-18.438-10.93-191.021 50.083 51-186.176-12.013-19.087c-50.525-80.336-77.198-173.175-77.16-268.504.111-278.186 226.507-504.503 504.898-504.503 134.812.056 261.519 52.604 356.814 147.965 95.289 95.36 147.728 222.128 147.688 356.948-.118 278.195-226.522 504.543-504.693 504.543z"></path><linearGradient id="a" gradientUnits="userSpaceOnUse" x1="609.77" y1="1190.114" x2="609.77" y2="21.084"><stop offset="0" stop-color="#20b038"></stop><stop offset="1" stop-color="#60d66a"></stop></linearGradient><path fill="url(#a)" d="M27.875 1190.114l82.211-300.18c-50.719-87.852-77.391-187.523-77.359-289.602.133-319.398 260.078-579.25 579.469-579.25 155.016.07 300.508 60.398 409.898 169.891 109.414 109.492 169.633 255.031 169.57 409.812-.133 319.406-260.094 579.281-579.445 579.281-.023 0 .016 0 0 0h-.258c-96.977-.031-192.266-24.375-276.898-70.5l-307.188 80.548z"></path><image overflow="visible" opacity=".08" width="682" height="639" xlink:href="FCC0802E2AF8A915.png" transform="translate(270.984 291.372)"></image><path fill-rule="evenodd" clip-rule="evenodd" fill="#FFF" d="M462.273 349.294c-11.234-24.977-23.062-25.477-33.75-25.914-8.742-.375-18.75-.352-28.742-.352-10 0-26.25 3.758-39.992 18.766-13.75 15.008-52.5 51.289-52.5 125.078 0 73.797 53.75 145.102 61.242 155.117 7.5 10 103.758 166.266 256.203 226.383 126.695 49.961 152.477 40.023 179.977 37.523s88.734-36.273 101.234-71.297c12.5-35.016 12.5-65.031 8.75-71.305-3.75-6.25-13.75-10-28.75-17.5s-88.734-43.789-102.484-48.789-23.75-7.5-33.75 7.516c-10 15-38.727 48.773-47.477 58.773-8.75 10.023-17.5 11.273-32.5 3.773-15-7.523-63.305-23.344-120.609-74.438-44.586-39.75-74.688-88.844-83.438-103.859-8.75-15-.938-23.125 6.586-30.602 6.734-6.719 15-17.508 22.5-26.266 7.484-8.758 9.984-15.008 14.984-25.008 5-10.016 2.5-18.773-1.25-26.273s-32.898-81.67-46.234-111.326z"></path><path fill="#FFF" d="M1036.898 176.091C923.562 62.677 772.859.185 612.297.114 281.43.114 12.172 269.286 12.039 600.137 12 705.896 39.633 809.13 92.156 900.13L7 1211.067l318.203-83.438c87.672 47.812 186.383 73.008 286.836 73.047h.255.003c330.812 0 600.109-269.219 600.25-600.055.055-160.343-62.328-311.108-175.649-424.53zm-424.601 923.242h-.195c-89.539-.047-177.344-24.086-253.93-69.531l-18.227-10.805-188.828 49.508 50.414-184.039-11.875-18.867c-49.945-79.414-76.312-171.188-76.273-265.422.109-274.992 223.906-498.711 499.102-498.711 133.266.055 258.516 52 352.719 146.266 94.195 94.266 146.031 219.578 145.992 352.852-.118 274.999-223.923 498.749-498.899 498.749z"></path></g></svg>',
            'setting_key' => 'enable_whatsapp_integration',
            'action_hook' => 'Joinotify/Settings/Tabs/Integrations/Whatsapp',
        );

        return $integrations;
    }


    /**
     * Declarative settings rendered in the WhatsApp integration modal.
     *
     * This is the single home of the Cloud API credentials. The API key gets a
     * dedicated control instead of a plain text input because pasting a key is
     * not the whole job: the key has to be proven against the API and the
     * account's numbers imported before the site can send anything.
     *
     * @since 2.4.0
     * @return array<int,array<string,mixed>>
     */
    public static function get_integration_settings() {
        $token = Helpers::cloud_api_token();

        return array(
            self::field_text(
                'whatsapp_cloud_api_token',
                esc_html__( 'Joinotify account', 'joinotify' ),
                esc_html__( 'Create a key for this site on the Joinotify panel and paste it here. Connecting validates the key and imports the WhatsApp numbers on your account.', 'joinotify' ),
                array(
                    'type' => 'joinotify-api-key',
                    'placeholder' => 'sk_live_...',
                    'component_props' => array(
                        'panel_url' => JOINOTIFY_PANEL_URL,
                        'api_host' => wp_parse_url( JOINOTIFY_CLOUD_API_BASE_URL, PHP_URL_HOST ),
                        // The key itself is write-only (see Settings\Registry::
                        // get_write_only_keys), so the control is told whether one
                        // is stored and what its public prefix is, never its value.
                        'connected' => '' !== $token,
                        'key_prefix' => '' !== $token ? substr( $token, 0, 14 ) : '',
                    ),
                )
            ),
            self::field_select(
                'whatsapp_transport',
                esc_html__( 'Message transport', 'joinotify' ),
                esc_html__( 'Which service delivers WhatsApp messages. "Automatic" uses the Cloud API whenever a key is available, otherwise the legacy Evolution relay.', 'joinotify' ),
                array(
                    array( 'value' => 'auto', 'label' => esc_html__( 'Automatic', 'joinotify' ) ),
                    array( 'value' => 'cloud', 'label' => esc_html__( 'Cloud API (official)', 'joinotify' ) ),
                    array( 'value' => 'evolution', 'label' => esc_html__( 'Evolution (legacy)', 'joinotify' ) ),
                ),
                array(
                    'default' => 'auto',
                )
            ),
            self::field_text(
                'whatsapp_phone_number_id',
                esc_html__( 'Phone number ID', 'joinotify' ),
                esc_html__( 'Default origin for messages. Filled in when you connect; change it only to send from a different number by default.', 'joinotify' ),
                array(
                    'placeholder' => '',
                    'autocomplete' => 'off',
                )
            ),
            self::field_text(
                'whatsapp_waba_id',
                esc_html__( 'WhatsApp Business Account ID', 'joinotify' ),
                esc_html__( 'Business account that owns the message templates. Filled in when you connect.', 'joinotify' ),
                array(
                    'placeholder' => '',
                    'autocomplete' => 'off',
                )
            ),
        );
    }


    /**
     * Brand logo rendered next to every WhatsApp action in the builder.
     *
     * Every step that ends up delivering a WhatsApp message carries the same
     * logo, so the canvas reads as one channel instead of a mix of generic
     * glyphs. Shared with the AI message and interactive message actions.
     *
     * @since 2.4.0
     * @return string Inline SVG markup.
     */
    public static function get_action_icon() {
        return '<svg class="icon icon-lg whatsapp" viewBox="-1.5 0 259 259" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid" fill="#000000"><g stroke-width="0"></g><g stroke-linecap="round" stroke-linejoin="round"></g><g> <g> <path d="M67.6631045,221.823373 L71.8484512,223.916047 C89.2873956,234.379413 108.819013,239.262318 128.350631,239.262318 L128.350631,239.262318 C189.735716,239.262318 239.959876,189.038158 239.959876,127.653073 C239.959876,98.3556467 228.101393,69.7557778 207.17466,48.8290445 C186.247927,27.9023111 158.345616,16.0438289 128.350631,16.0438289 C66.9655467,16.0438289 16.7413867,66.2679889 17.4389445,128.350631 C17.4389445,149.277365 23.7169645,169.50654 34.1803311,186.945485 L36.9705622,191.130831 L25.8096378,232.28674 L67.6631045,221.823373 Z" fill="#00E676"> </path> <path d="M219.033142,37.66812 C195.316178,13.2535978 162.530962,0 129.048189,0 C57.8972956,0 0.697557778,57.8972956 1.39511556,128.350631 C1.39511556,150.67248 7.67313556,172.296771 18.1365022,191.828389 L0,258.096378 L67.6631045,240.657433 C86.4971645,251.1208 107.423898,256.003705 128.350631,256.003705 L128.350631,256.003705 C198.803967,256.003705 256.003705,198.106409 256.003705,127.653073 C256.003705,93.4727423 242.750107,61.3850845 219.033142,37.66812 Z M129.048189,234.379413 L129.048189,234.379413 C110.214129,234.379413 91.380069,229.496509 75.3362401,219.7307 L71.1508934,217.638027 L30.6925422,228.101393 L41.1559089,188.3406 L38.3656778,184.155253 C7.67313556,134.628651 22.3218489,69.05822 72.5460089,38.3656778 C122.770169,7.67313556 187.643042,22.3218489 218.335585,72.5460089 C249.028127,122.770169 234.379413,187.643042 184.155253,218.335585 C168.111425,228.798951 148.579807,234.379413 129.048189,234.379413 Z M190.433273,156.9505 L182.760138,153.462711 C182.760138,153.462711 171.599213,148.579807 164.623636,145.092018 C163.926078,145.092018 163.22852,144.39446 162.530962,144.39446 C160.438289,144.39446 159.043173,145.092018 157.648058,145.789576 L157.648058,145.789576 C157.648058,145.789576 156.9505,146.487133 147.184691,157.648058 C146.487133,159.043173 145.092018,159.740731 143.696902,159.740731 L142.999345,159.740731 C142.301787,159.740731 140.906671,159.043173 140.209113,158.345616 L136.721325,156.9505 L136.721325,156.9505 C129.048189,153.462711 122.072611,149.277365 116.492149,143.696902 C115.097033,142.301787 113.00436,140.906671 111.609245,139.511556 C106.72634,134.628651 101.843436,129.048189 98.3556467,122.770169 L97.658089,121.375053 C96.9605312,120.677496 96.9605312,119.979938 96.2629734,118.584822 C96.2629734,117.189707 96.2629734,115.794591 96.9605312,115.097033 C96.9605312,115.097033 99.7507623,111.609245 101.843436,109.516571 C103.238551,108.121456 103.936109,106.028782 105.331225,104.633667 C106.72634,102.540993 107.423898,99.7507623 106.72634,97.658089 C106.028782,94.1703001 97.658089,75.3362401 95.5654156,71.1508934 C94.1703001,69.05822 92.7751845,68.3606623 90.6825112,67.6631045 L88.5898378,67.6631045 C87.1947223,67.6631045 85.1020489,67.6631045 83.0093756,67.6631045 C81.6142601,67.6631045 80.2191445,68.3606623 78.8240289,68.3606623 L78.1264712,69.05822 C76.7313556,69.7557778 75.3362401,71.1508934 73.9411245,71.8484512 C72.5460089,73.2435667 71.8484512,74.6386823 70.4533356,76.0337978 C65.5704312,82.3118178 62.7802,89.9849534 62.7802,97.658089 L62.7802,97.658089 C62.7802,103.238551 64.1753156,108.819013 66.2679889,113.701918 L66.9655467,115.794591 C73.2435667,129.048189 81.6142601,140.906671 92.7751845,151.370038 L95.5654156,154.160269 C97.658089,156.252942 99.7507623,157.648058 101.145878,159.740731 C115.794591,172.296771 132.535978,181.365022 151.370038,186.247927 C153.462711,186.945485 156.252942,186.945485 158.345616,187.643042 L158.345616,187.643042 C160.438289,187.643042 163.22852,187.643042 165.321193,187.643042 C168.808982,187.643042 172.994329,186.247927 175.78456,184.852811 C177.877233,183.457696 179.272349,183.457696 180.667465,182.06258 L182.06258,180.667465 C183.457696,179.272349 184.852811,178.574791 186.247927,177.179676 C187.643042,175.78456 189.038158,174.389445 189.735716,172.994329 C191.130831,170.204098 191.828389,166.716309 192.525947,163.22852 C192.525947,161.833405 192.525947,159.740731 192.525947,158.345616 C192.525947,158.345616 191.828389,157.648058 190.433273,156.9505 Z" fill="#FFFFFF"></path></g></g></svg>';
    }


    /**
     * Add WhatsApp messages actions in sidebar list on builder
     *
     * @since 1.1.0
     * @version 1.4.7
     * @param array $actions | Current actions
     * @return array
     */
    public function add_whatsapp_messages( $actions ) {
        $actions[] = array(
            'action' => 'send_whatsapp_message_text',
            'title' => __( 'WhatsApp: Text message', 'joinotify' ),
            'description' => __( 'Send a text message via WhatsApp.', 'joinotify' ),
            'context' => array(),
            'category' => 'messages',
            'icon' => self::get_action_icon(),
            'external_icon' => false,
            'has_settings' => true,
            'priority' => 40,
            'is_expansible' => true,
        );

        $actions[] = array(
            'action' => 'send_whatsapp_message_media',
            'title' => __( 'WhatsApp: Media message', 'joinotify' ),
            'description' => __( 'Send a media message (image, file, document, and audio) with WhatsApp.', 'joinotify' ),
            'context' => array(),
            'category' => 'messages',
            'icon' => self::get_action_icon(),
            'external_icon' => false,
            'has_settings' => true,
            'priority' => 50,
            'is_expansible' => true,
        );

        // Approved templates are the only way to reach someone outside the
        // 24-hour session window, which is where business initiated workflows
        // almost always run. Cloud API transport only — the handler skips the
        // action (with a log) when the legacy relay is active.
        $actions[] = array(
            'action' => 'send_whatsapp_message_template',
            'title' => __( 'WhatsApp: Template message', 'joinotify' ),
            'description' => __( 'Send an approved template through the official WhatsApp Cloud API. Required to start a conversation or reply outside the 24-hour window.', 'joinotify' ),
            'context' => array(),
            'category' => 'messages',
            'icon' => self::get_action_icon(),
            'external_icon' => false,
            'has_settings' => true,
            'priority' => 45,
            'is_expansible' => true,
            'default_data' => array(
                'title' => __( 'WhatsApp: Template message', 'joinotify' ),
                'description' => '',
                'action' => 'send_whatsapp_message_template',
                'sender' => '',
                'receiver' => '{{ wc_billing_phone }}',
                'template_name' => '',
                'language' => 'pt_BR',
                'variables' => array(),
            ),
        );

        return $actions;
    }
}