<?php

/**
 * Builder view file.
 *
 * @since 1.4.7
 * @version 2.0.0
 */

defined('ABSPATH') || exit;

$debug_mode = defined( 'JOINOTIFY_DEBUG_MODE' ) ? (bool) JOINOTIFY_DEBUG_MODE : false; ?>

<div class="wrap p-0 joinotify-builder-page">
	<style>
		#adminmenumain,
		#wpfooter {
			display: none !important;
		}

		<?php if ( ! $debug_mode ) : ?>
			#wpadminbar {
				display: none !important;
			}
		<?php endif; ?>
	</style>

	<div id="joinotify-builder-app" class="joinotify-builder-app">
		<style>
			#adminmenumain,
			#wpfooter {
				display: none !important;
			}

			<?php if ( ! $debug_mode ) : ?>
				#wpadminbar {
					display: none !important;
				}
			<?php endif; ?>

			/*
			 * Inline pre-mount loader. Mirrors the Vue BuilderLoader spinner
			 * (app/src/pages/builder/styles.css) so the server-rendered loader
			 * and the Vue one are visually identical, making the hand-off on
			 * mount seamless (a single, continuous loader). This style block and
			 * markup live inside #joinotify-builder-app, so Vue removes them when
			 * it mounts and replaces the element's content.
			 */
			.flex {
				display: flex;
			}

			.d-block {
				display: block;
			}

			.align-items-center {
				align-items: center;
			}

			.justify-content-center {
				justify-content: center;
			}

			.position-absolute {
				position: absolute;
			}

			.joinotify-boot-loader {
				position: fixed;
				inset: 0;
				z-index: 9999999;
				display: flex;
				align-items: center;
				justify-content: center;
				background-color: #fff;
			}

			.joinotify-boot-loader__stack {
				display: flex;
				flex-direction: column;
				align-items: center;
				padding: 0 1rem;
			}

			.joinotify-loader-container {
				position: relative;
				width: 6rem;
				height: 6rem;
				display: inline-flex;
				align-items: center;
				justify-content: center;
				color: var(--joinotify-success, #22c55e);
			}

			.joinotify-loader {
				width: 6rem;
				height: 6rem;
				display: inline-block;
				color: var(--joinotify-success, #22c55e);
				-webkit-animation: transform-animation-circular 1.4s linear infinite;
				animation: transform-animation-circular 1.4s linear infinite;
			}

			.circular-progress-animation {
				stroke: currentColor;
				stroke-dasharray: 80px, 200px;
				stroke-dashoffset: 0;
				stroke-width: 3px;
				-webkit-animation: stroke-animation-circular 1.4s ease-in-out infinite;
				animation: stroke-animation-circular 1.4s ease-in-out infinite;
			}

			@-webkit-keyframes stroke-animation-circular {
				0% {
					stroke-dasharray: 1px, 200px;
					stroke-dashoffset: 0;
				}

				50% {
					stroke-dasharray: 100px, 200px;
					stroke-dashoffset: -15px;
				}

				100% {
					stroke-dasharray: 100px, 200px;
					stroke-dashoffset: -125px;
				}
			}

			@keyframes stroke-animation-circular {
				0% {
					stroke-dasharray: 1px, 200px;
					stroke-dashoffset: 0;
				}

				50% {
					stroke-dasharray: 100px, 200px;
					stroke-dashoffset: -15px;
				}

				100% {
					stroke-dasharray: 100px, 200px;
					stroke-dashoffset: -125px;
				}
			}

			@keyframes transform-animation-circular {
				0% {
					-webkit-transform: rotate(0deg);
					-moz-transform: rotate(0deg);
					-ms-transform: rotate(0deg);
					transform: rotate(0deg);
				}

				100% {
					-webkit-transform: rotate(360deg);
					-moz-transform: rotate(360deg);
					-ms-transform: rotate(360deg);
					transform: rotate(360deg);
				}
			}

			@-webkit-keyframes transform-animation-circular {
				0% {
					-webkit-transform: rotate(0deg);
					-moz-transform: rotate(0deg);
					-ms-transform: rotate(0deg);
					transform: rotate(0deg);
				}

				100% {
					-webkit-transform: rotate(360deg);
					-moz-transform: rotate(360deg);
					-ms-transform: rotate(360deg);
					transform: rotate(360deg);
				}
			}

			#joinotify-builder-logo {
				width: 2.825rem;
			}

			.joinotify-loader-container .circle-base {
				position: absolute;
				z-index: -1;
				width: 6rem;
			}

			.joinotify-loader-container .circle-base circle {
				stroke-width: 3px;
				position: absolute;
				stroke: #CED4DA;
			}

			.joinotify-loader-text {
				margin-top: 1.5rem;
				font-size: 0.875rem;
				font-weight: 500;
				line-height: 1.5rem;
				color: #475569;
				text-align: center;
			}
		</style>

		<div class="joinotify-boot-loader">
			<div class="joinotify-boot-loader__stack">
				<div class="flex align-items-center justify-content-center joinotify-loader-container">
					<svg id="joinotify-builder-logo" class="loader-logo position-absolute" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 703 882.5">
						<path d="M908.66,248V666a126.5,126.5,0,0,1-207.21,97.41l-16.7-16.7L434.08,496.07l-62-62a47.19,47.19,0,0,0-72,30.86V843.36a47.52,47.52,0,0,0,69.57,35.22l19.3-19.3,56-56,81.19-81.19,10.44-10.44a47.65,47.65,0,0,1,67.63,65.05l-13,13L428.84,952.12l-9.59,9.59a128,128,0,0,1-213.59-95.18V413.17a124.52,124.52,0,0,1,199.78-82.54l22.13,22.13L674.45,599.64l46.22,46.22,17,17a47.8,47.8,0,0,0,71-31.44V270.19a48.19,48.19,0,0,0-75-40.05L720.43,243.4l-68.09,68.09L575.7,388.13a48.39,48.39,0,0,1-67.43-67.93L680,148.46A136,136,0,0,1,908.66,248Z" transform="translate(-205.66 -112.03)" style="fill:#22c55e;"></path>
					</svg>

					<span class="joinotify-circular-progress joinotify-loader" role="progressbar">
						<svg class="loader-circular-progress d-block" viewBox="22 22 44 44">
							<circle class="circular-progress-animation" cx="44" cy="44" r="20.2" fill="none"></circle>
						</svg>
					</span>

					<svg class="circle-base" viewBox="22 22 44 44">
						<circle cx="44" cy="44" r="20.2" fill="none"></circle>
					</svg>
				</div>

				<p class="joinotify-loader-text"><?php echo esc_html__( 'Initializing builder', 'joinotify' ); ?></p>
			</div>
		</div>
	</div>
</div>
